"use client";

import { useState } from "react"; // 1. Added useState
import { Heart } from "lucide-react";
import { Button } from "@/shared/components/ui/button";
import ProductRating from "./ProductRating";

interface Props {
  name: string;
  category?: string;
  price: string;
  description?: string;
  rating?: number;
  reviewCount?: number;
  inStock?: boolean;
  isWishlisted?: boolean;
  onWishlistToggle?: () => void;
  wishlistLoading?: boolean;
}

export default function ProductInfo({
  name,
  category,
  price,
  description,
  rating = 4,
  reviewCount = 150,
  inStock = true,
  isWishlisted = false,
  onWishlistToggle,
  wishlistLoading = false,
}: Props) {
  // 2. Added state to handle expansion
  const [isExpanded, setIsExpanded] = useState(false);

  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          {category ? (
            <span className="inline-block rounded-md border border-white/8 bg-white/5 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-widest text-zinc-400">
              {category}
            </span>
          ) : null}

          {/* 3. Updated H1 with line-clamp and toggle logic */}
          <div className="mt-2.5">
            <h1 
              className={`text-2xl font-bold leading-tight tracking-tight text-white sm:text-3xl lg:text-[2rem] ${!isExpanded ? "line-clamp-2" : ""}`}
            >
              {name}
            </h1>
            {name.length > 60 && (
              <button 
                onClick={() => setIsExpanded(!isExpanded)}
                className="text-xs text-[#DB4444] font-medium mt-1 hover:underline"
              >
                {isExpanded ? "Show less" : "...read more"}
              </button>
            )}
          </div>
        </div>

        {/* ... Wishlist Button remains same ... */}
        {onWishlistToggle ? (
          <Button
            type="button"
            variant="outline"
            onClick={onWishlistToggle}
            disabled={wishlistLoading}
            className={[
              "h-12 rounded-xl border-white/10 bg-black/20 text-white hover:bg-white/5",
              isWishlisted ? "border-red-500/40 text-red-400" : "",
            ].join(" ")}
          >
            <Heart className={`mr-2 h-4 w-4 ${isWishlisted ? "fill-red-500 text-red-500" : ""}`} />
            {isWishlisted ? "Remove" : "Wishlist"}
          </Button>
        ) : null}
      </div>

      {/* ... Rating + Stock ... */}
      <div className="flex flex-wrap items-center gap-3">
        <ProductRating rating={rating} reviewCount={reviewCount} />
        <span className="h-3.5 w-px bg-white/15" />
        <span className={["text-xs font-semibold", inStock ? "text-emerald-400" : "text-red-400"].join(" ")}>
          {inStock ? "● In Stock" : "● Out of Stock"}
        </span>
      </div>

      <div>
        <span className="text-3xl font-bold tracking-tight text-white">{price}</span>
      </div>

      <div className="h-px w-full bg-gradient-to-r from-white/10 via-white/5 to-transparent" />

      <p className="text-sm leading-relaxed text-zinc-400 sm:text-[0.9375rem]">
        {description || "High quality product with a clean, modern finish and reliable everyday performance."}
      </p>
    </div>
  );
}